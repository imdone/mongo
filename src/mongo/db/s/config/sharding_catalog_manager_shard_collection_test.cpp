...
TEST_F(ShardCollectionTest, noInitialChunksOrData) {
    ...
    // TODO SERVER-29451: add hooks to the mock storage engine to expect reads and writes. id:714
...
TEST_F(ShardCollectionTest, withInitialChunks) {
    ...
    // Now start actually sharding the collection.
    auto future = launchAsync([&] {
        // TODO: can we mock the ShardRegistry to return these? id:1297
    ...
    // We do not check for a specific ChunkVersion, because we cannot easily know the OID that was
    // generated by shardCollection for the first chunk.
    // TODO SERVER-29451: add hooks to the mock storage engine to expect reads and writes. id:654
...
TEST_F(ShardCollectionTest, withInitialData) {
    ...
    // We do not check for a specific ChunkVersion, because we cannot easily know the OID that was
    // generated by shardCollection for the first chunk.
    // TODO SERVER-29451: add hooks to the mock storage engine to expect reads and writes. id:1886
