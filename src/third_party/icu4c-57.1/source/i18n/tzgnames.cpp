    ...
    if (!mzID.isEmpty()) {
        ...
        if (useStandard) {
            ...
            if (!stdName.isEmpty()) {
                ...
                // TODO: revisit this issue later id:1830
...
TZGNCore::findBestMatch(const UnicodeString& text, int32_t start, uint32_t types,
    ...
    UBool isStandard = FALSE;       // TODO: Temporary hack (on hack) for short standard name/location name conflict (found in zh_Hant), should be removed after CLDR 21m1 integration id:2583
    ...
    if (tznamesMatches != NULL) {
        ...
        for (int32_t i = 0; i < tznamesMatches->size(); i++) {
            ...
            if (len > bestMatchLen) {
                ...
                case UTZNM_SHORT_STANDARD:  // this one is never used for generic, but just in case
                    isStandard = TRUE;      // TODO: Remove this later, see the comments above. id:2185
        ...
        if (bestMatchLen == (text.length() - start)) {
            ...
            // TODO Some time zone uses a same name for the long standard name id:3293
...
*/
            // TODO The deprecation of commonlyUsed flag introduced the name id:2903
    ...
    if (localMatches != NULL) {
        for (int32_t i = 0; i < localMatches->size(); i++) {
            ...
            // TODO See the above TODO. We use len >= bestMatchLen id:1832
