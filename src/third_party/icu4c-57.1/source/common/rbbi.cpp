...
RuleBasedBreakIterator::operator=(const RuleBasedBreakIterator& that) {
    ...
    // TODO: clone fLanguageBreakEngines from "that" id:3222
...
RuleBasedBreakIterator::operator==(const BreakIterator& that) const {
    ...
    // TODO: need a check for when in a dictionary region at different offsets. id:2773
...
RuleBasedBreakIterator::getRules() const {
    if (fData != NULL) {
        ...
        if (s == NULL) {
            // TODO: something more elegant here. id:1627
...
int32_t RuleBasedBreakIterator::previous(void) {
    ...
    // if we're already sitting at the beginning of the text, return DONE  id:2413
...
int32_t RuleBasedBreakIterator::following(int32_t offset) {
    ...
    // just return DONE ; if it's before the beginning, return the id:1905
    ...
    // TODO: could use binary search id:3223
    // TODO: what if offset is outside range, but break is not? id:2775
    if (fCachedBreakPositions != NULL) {
    ...
    if (fData->fSafeRevTable != NULL) {
        ...
        // TODO: is this still needed, with move to code point boundary handled above? id:1629
...
int32_t RuleBasedBreakIterator::preceding(int32_t offset) {
    // if the offset passed in is already past the end of the text,
    // just return DONE ; if it's before the beginning, return the id:2415
    // text's starting offset
    if (fText == NULL || offset > utext_nativeLength(fText)) {
        return last();
    }
    ...
    // covered by them, use them
    if (fCachedBreakPositions != NULL) {
        // TODO: binary search? id:1908
        // TODO: What if offset is outside range, but break is not? id:3224
        if (offset > fCachedBreakPositions[0]
                && offset <= fCachedBreakPositions[fNumCachedBreakPositions - 1]) {
            fPositionInCache = 0;
    ...
    if (fData->fSafeFwdTable != NULL) {
        ...
        // TODO: (synwee) would it be better to just check for being in the middle of a surrogate pair, id:2777
        //        rather than adjusting the position unconditionally?
        //        (Change would interact with safe rules.)
        // TODO: change RBBI behavior for off-boundary indices to match that of UText? id:1631
    ...
    if (fData->fSafeRevTable != NULL) {
        // backup plan if forward safe table is not available
        //  TODO: check whether this path can be discarded id:2417
...
int32_t RuleBasedBreakIterator::handleNext(const RBBIStateTable *statetable) {
    ...
    // if we're already at the end of the text, return DONE . id:1911
...
int32_t RuleBasedBreakIterator::handlePrevious(const RBBIStateTable *statetable) {
    ...
    // if we're already at the start of the text, return DONE . id:3225
        ...
        if (c == U_SENTINEL) {
            ...
            if (mode == RBBI_END) {
                ...
                if (result == initialPosition) {
                    ...
                    UTEXT_SETNATIVEINDEX(fText, initialPosition);
                    (void)UTEXT_PREVIOUS32(fText);   // TODO: shouldn't be necessary. We're already at beginning. Check. id:2779
                }
                break;
            }
            // Run the loop one last time with the fake end-of-input character category.
...
int32_t RuleBasedBreakIterator::checkDictionary(int32_t startPos,
    ...
    if (category & 0x4000) {
        if (reverse) {
            do {
                utext_next32(fText);          // TODO: recast to work directly with postincrement. id:1633
            ...
            if (c == U_SENTINEL) {
                // c = fText->last32();
                //   TODO: why was this if needed? id:2419
                c = UTEXT_PREVIOUS32(fText);
            }
            else {
                c = UTEXT_PREVIOUS32(fText);
    ...
    while(U_SUCCESS(status)) {
        while((current = (int32_t)UTEXT_GETNATIVEINDEX(fText)) < rangeEnd && (category & 0x4000) == 0) {
            utext_next32(fText);           // TODO: tweak for post-increment operation id:1914
            c = utext_current32(fText);
            UTRIE_GET16(&fData->fTrie, c, category);
        }
        if (current >= rangeEnd) {
